...
{
    size_t sendingBatchSize = stringBufferToView( sharedStateSnapshot->firstDataToSendNode->serializedEvents ).length;
    ELASTIC_APM_BACKGROUND_BACKEND_COMM_DO_UNDER_LOCK_PROLOG()

    backgroundBackendComm->isSendingBatch = true;
    backgroundBackendComm->sendingBatchId = sharedStateSnapshot->firstDataToSendNode->id;
    backgroundBackendComm->sendingBatchSize = sendingBatchSize;
        ...
        , const BackgroundBackendCommSharedStateSnapshot* sharedStateSnapshot )
{
    ...
    resultCode = backgroundBackendCommThreadFunc_markFirstEventsBatchAsInFlight( backgroundBackendComm, sharedStateSnapshot );
    ...
    resultCode = syncSendEventsToApmServer( config
                                            ...
                                            , serializedEvents );
    ResultCode sharedStateResultCode = backgroundBackendCommThreadFunc_getSharedStateSnapshot( backgroundBackendComm, /* out */ &currentSharedStateSnapshot );
    if ( sharedStateResultCode != resultSuccess )
    {
        return sharedStateResultCode;
    }
